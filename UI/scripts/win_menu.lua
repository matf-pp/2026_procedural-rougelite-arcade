...
function win_menu.load(onLobby)
    onLobbyCallback = onLobby
...
local function drawMenuItems(w, h)
    ...
    local titleW = fontTitle:getWidth("YOU WIN")
    love.graphics.print("YOU WIN", math.floor(w / 2 - titleW / 2), math.floor(h * 0.25))
...
function win_menu.draw(bgCanvas)
...
function win_menu.keypressed(key, scancode, isrepeat)
...
function win_menu.mousemoved(x, y, dx, dy, istouch)
    local w = love.graphics.getWidth()
    local h = love.graphics.getHeight()
    for i = 1, #items do
...
function win_menu.mousepressed(x, y, button, istouch, presses)
    if button == 1 then
        local w = love.graphics.getWidth()
        local h = love.graphics.getHeight()
        for i = 1, #items do
